    ...
    if (jl_options.incremental && jl_generating_output()) {
        if (!ptrhash_has(&jl_current_modules, (void*)m) && !jl_is__toplevel__mod(m)) {
            if (m != jl_main_module) { // TODO: this was grand-fathered in
                ...
                jl_errorf("Eval into closed module `%s` breaks incremental compilation "
                          "becuase the side effects of the `eval` will not be permanent. "
                          "This is likely due to some other module mutating `%s` during "
                          "precompilation - don't do this.", name, name);
            }
        }
    }
    JL_TRY {
