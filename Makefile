...
ifeq ($(OS),WINNT)
	@echo '@"%~dp0"\'"$(shell $(PYTHON) $(call python_cygpath,$(JULIAHOME)/contrib/relative_path.py) "$(BUILDROOT)" "$(JULIA_EXECUTABLE)" | tr / '\\')" '%*' > $(BUILDROOT)/julia.bat
	chmod a+x $(BUILDROOT)/julia.bat
else
	@ln -sf "$(shell $(PYTHON) $(call python_cygpath,$(JULIAHOME)/contrib/relative_path.py) "$(BUILDROOT)" "$(JULIA_EXECUTABLE)")" $(BUILDROOT)/julia
endif
